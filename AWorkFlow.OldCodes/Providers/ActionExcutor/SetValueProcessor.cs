...
namespace Mcs.SF.WorkFlow.Api.Providers.ActionExcutor
{
    ...
    public class SetValueProcessor : IVariableProcessor
    {
        ...
        /// <returns></returns>
        public Task<ActionExecuteResult> Execute(VariableProcessActionSetting setting, ArgumentProvider argument)
        {
            ...
            foreach (var kvp in setting.Output)
            {
                ...
                if (obj == null)
                {
                    argument.Put(target, argument.Format(kvp.Value));
                ...
                else
                {
                    ...
                    argument.Put(currentKey, JsonConvert.SerializeObject(token));
